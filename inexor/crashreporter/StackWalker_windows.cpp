...
#define CALLSTACK_STARTFRAME 3
#define CALLSTACK_NUMFRAMES 10
...
StackWalker::StackWalker(char *output, int outputlen, int options, LPCSTR szSymPath, DWORD dwProcessId, HANDLE hProcess)
{
  this->output = output;
...
void StackWalker::OnLoadModule(LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size, DWORD result, LPCSTR symType, LPCSTR pdbName)
{
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry, int frameNUM)
{
  CHAR buffer[STACKWALK_MAX_NAMELEN];
...
{
    strcat_s(output, outputlen, buffer);
}
